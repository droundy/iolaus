...
func peMap1(f func(PatienceElem) PatienceElem, slice []PatienceElem) {
	for i,v := range slice {
		slice[i] = f(v)
	}
}
...
func peMap(f func(PatienceElem) PatienceElem, slice []PatienceElem) []PatienceElem {
...
func peFold(f func(PatienceElem, PatienceElem) PatienceElem, x PatienceElem, slice []PatienceElem) PatienceElem {
  for _, v := range slice {
    x = f(x, v)
  }
  return x
...
func peFilter(f func(PatienceElem) bool, slice []PatienceElem) []PatienceElem {
...
// expanding if needed.
func peAppend(slice []PatienceElem, val PatienceElem) []PatienceElem {
	length := len(slice)
	if cap(slice) == length {
		// we need to expand
...
func peRepeat(val PatienceElem, n int) []PatienceElem {
...
func peCat(slices ...[]PatienceElem) []PatienceElem {
	return peCats(slices)
}

// Cats concatenates several slices, expanding if needed.
func peCats(slices [][]PatienceElem) []PatienceElem {
	lentot := 0
...
func peReverse(slice []PatienceElem) (out []PatienceElem) {
	ln := len(slice)
...
func peAny(f func(PatienceElem) bool, slice []PatienceElem) bool {
	for _,v:= range slice {
		if f(v) { return true }
	}
	return false
...
func petestTypes(arg0 PatienceElem, arg1 PatienceElem) {
